...
function sumOfMultiples() {
    ...
    var target = parseInt(document.forms["numberFun"]["num3"].value);

    document.getElementById("results").style.display = "block";

    document.getElementById("textAppear").innerText = "Here is the sum of multiples of " + num1 + " or " + num2 + " below " + target + ":";


    //Rows 70 through 85 are the first solution I created for this problem. This algorithm works, but its memory requirements increase as the target variable increases.
    //for (i = 1; i < target; i++) {
    //
    ...
    //    return a + b;
    //}
    //Rows 87-93 are the second solution that I created for this problem. Its memory usage does not increase as the target variable increases.  However, this solution is inefficient for large numbers.
//        var sumMultiples = 0;
//    
//        for (i = 1; i < target; i++) {
//            if (i % num1 == 0 || i % num2 == 0) {
...
//        }
//    
// The following solution is more efficient for large numbers. I translated this solution into JavaScript from a solution given on projecteuler.net.
    target = target - 1;
    var n;
  ...
  return Math.floor (n*(p*(p+1)) / 2);
    ...
    var sumMultiples = SumDivisibleBy(num1) + SumDivisibleBy(num2) - SumDivisibleBy(num1 * num2);
